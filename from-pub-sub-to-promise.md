...
这一段应该是最好理解的了。那么接下来，我们从发布/订阅模式来看看then函数所处的位置和作用。1.1中的图描述了事件在发布/订阅模式中的流向。接下来，让我们从依赖关系的角度看看这个模式中各个成员的关系。

很明显，`then`函数建立了订阅者和调度中心的一种单向的数据(事件)流动。同时，在整个周期中，订阅者并不知道事件是从哪来，何时回来，只知道会从订阅中心来。这样从而充分的将订阅者和发布者解耦。
