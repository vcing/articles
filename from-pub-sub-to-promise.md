...
- #### 内部回调函数
    ...
    `Promise`对象内部定义的回调函数，标准规范中对应的`resolve`和`reject`函数。本文简化后只保留`resolve`函数。`resolve`函数执行时有且只能带有一个参数。发布函数执行时将把内部回调函数作为参数传入发布函数。在1.3代码中为`Promise`内部的`resolve`函数。
