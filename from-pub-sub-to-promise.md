...
### 术语(自己编的)
- [^1]: **发布者**：事件的发布者，在本场景中为同桌

- [^2]: **订阅者**：事件的订阅者，在本场景中为我

- [^3]: **thenable对象**：JS鸭子类型特点，如果该对象有then函数(即thenable)，我们就将视它为Promise对象

- [^4]: **内部回调函数**：`Promise`对象内部定义的回调函数，标准规范中对应的`resolve`和`reject`函数。本文简化后只保留`resolve`函数。`resolve`函数执行时有且只能带有一个参数。发布函数执行时将把内部回调函数作为参数传入发布函数。在1.3代码中为`Promise`内部的`resolve`函数。

- **发布函数**：发布者在创建`Promise`对象的时候，传入构造函数的参数，该参数有且只有一个，并且该参数为一个函数，即发布函数。标准Promise规范中，发布函数应有两个参数，分别为发布函数内部执行成功(完成)时和执行失败(拒绝)时所需调用的对应回调函数。但在本文中简化为只有一个参数，即发布函数内部执行成功(完成)回调函数。发布函数将在`Promise`对象初始化后立即执行并带上`Promise`对象的内部回调函数[^4]作为参数。在1.3代码中为`Promise`构造函数的`fn`。

- **订阅回调函数**：订阅者在通过`then`函数订阅该事件时传入的函数参数，标准Promise规范中为两个参数，第一个为事件完成时(fulfilled)执行的回调，第二个为事件拒绝时(rejected)执行的回调。本文简化后只保留`onFulfilled`函数。对应1.3中`than`函数的形参`onFulfilled`和1.4场景中的`me.wakeUp`作为实参。

- **外部Promise**：执行`teachWarning()`返回的`Promise`对象，因为是执行后直接返回的，即不属于订阅者也不属于发布者，并且在Promise链式调用和内部Promise递归中也属于最外层，所以暂且先这么称呼它。

- **链式Promise第n层**：Promise的链式调用中第n个`then()`函数返回的promise对象。所以外部Promise也可以叫做链式Promise第0层。

- **递归Promise第n层**：发布函数执行内部回调函数`resolve`所带的参数，如果该参数为一个thenable对象，那么我将称他为递归Promise第n层。所以如果外部Promise第一次执行内部回调函数`resolve`时所接收到的参数为一个thenable对象，它便称为递归Promise第1层。因为这其中确实是会将Promise的`then`方法递归调用
